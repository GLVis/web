...
```
In other words, the actual mesh and solution file names are obtained from the above prefixes "`mesh`" and "`sol`" by appending "`.`" followed by a 6-digit processor/subdomain number padded with 0's, see e.g. [this example](https://github.com/mfem/mfem/blob/master/examples/ex1p.cpp#L222). (Note that the related obsolete option -par3d was removed in version 2.0).
...
```
An example of this can be found [here](https://github.com/mfem/mfem/blob/master/examples/ex1p.cpp#L240).
